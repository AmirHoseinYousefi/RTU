...
bool relayStateOut[8] = {1 ,1 ,1 ,1 ,1 ,1 ,1 ,1};
bool Last_relayStateOut[8] = {0 ,0 ,0 ,0 ,0 ,0 ,0 ,0};
...
void logDigitalOut(void) {
  ...
  for (size_t i = 0; i < 8; i++) {
    Serial.printf("Channel %d : " ,i); Serial.println(Last_relayStateOut[i]);
  }
  Serial.println("--> ********************************** <--");
  Serial.println("");
}
...
ModbusMessage FC02(ModbusMessage request) {
  ...
  // Serial.println("READ_DISCR_INPUT");
...
ModbusMessage FC03(ModbusMessage request) {
  ...
  // Serial.println("READ_HOLD_REGISTER");
...
ModbusMessage FC05(ModbusMessage request) {
  ...
  Serial.print("Modbus --> relayStateOut "); Serial.println(relayStateOut[(7 - address)]);
...
ModbusMessage FC06(ModbusMessage request) {
  ...
  // Serial.println("WRITE_HOLD_REGISTER");
...
ModbusMessage FC0F(ModbusMessage request) {
  ...
  // Serial.println("WRITE_MULT_COILS");
...
public:
    void setup() {
      ...
      // if (Ethernet.begin(mac ,15000 ,5000) == 0) {
      ...
      // MB.registerWorker(0x01, READ_COIL, &FC01);
      // MB.registerWorker(0x01, WRITE_COIL, &FC05);
      // MB.registerWorker(0x01, WRITE_MULT_COILS, &FC0F);

      // MB.registerWorker(0x01, READ_HOLD_REGISTER, &FC03);
      // MB.registerWorker(0x01, WRITE_HOLD_REGISTER, &FC06);
      // MB.registerWorker(0x01, WRITE_MULT_REGISTERS, &FC10);

      // MB.registerWorker(0x01, READ_DISCR_INPUT, &FC02);

      // MB.registerWorker(0x01, READ_INPUT_REGISTER, &FC04);
      // MB.start(PORT, 2, 2000 ,0);
    }
